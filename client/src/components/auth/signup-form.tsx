...
export function SignupForm() {
  ...
  // Step 1: send OTP — password hashed server-side and stored in OTP record
  ...
  const handleVerifyOTP = (e: React.FormEvent) => {
    ...
    setError(null);
    startTransition(async () => {
      const result = await verifyOTPAction(email, otp, undefined, rememberMe);
      ...
      <div className="rounded-2xl border border-border bg-card p-5 sm:p-8 shadow-2xl">
        ...
        <div className="mb-7 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            {step === "details"
              ? "Start managing AI agent access on-chain."
